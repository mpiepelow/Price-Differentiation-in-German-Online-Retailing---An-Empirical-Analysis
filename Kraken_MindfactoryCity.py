...
with open('linkliste_sample Mindfactory.csv', 'r') as file:
    ...
    output = open(date1 + " Mindfactory_MFC.csv", "w+")
    ...
    driver.get("https://www.mindfactory-city.de/")
    time.sleep(4)
    ...
    for a in linklist_all:
        try:
            ...
            link_product = str(a).replace("https://www.mindfactory.de/","https://www.mindfactory-city.de/")
            ...
            Ean = driver.find_elements_by_xpath("//script[contains(.,'window.onload = function(){')]")
            for number in Ean:
                try:
