...
with open('datasample_zurrose.csv', 'r') as file:
    ...
    # create file
    output = open(date1 + " zurrose_zurrose.csv", "w+")
    ...
    driver.get("https://www.zurrose.de/")

    time.sleep(5)
    
    for a in list_links_all:
        try:
            linklist = str("https://www.zurrose.de/catalogsearch/result/?q=") + str(a)
            #time.sleep(1)
            ...
            product_pages = driver.find_elements_by_xpath("//meta[@property='product:price:amount']")
...
driver.close()
        ...
        #    list_links_all.append(p)
            ...
            #pass
    #print(list_links_all)
    #print(len(list_links_all))
    #sample = random.choices(list_links_all, k=1000)
    #file = open("datasaample_zurrose.csv", "w+")
